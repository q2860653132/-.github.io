...
async function loadFromGitHub() {
    try {
        ...
        if (response.status === 404) {
            ...
            console.log('Loaded data body:', body);
...
async function saveToGitHub(data) {
    try {
        ...
        console.log('Stringified data:', body);
        
        // 先获取 issue 信息
        const getResponse = await fetch(`https://api.github.com/repos/${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo}/issues/1`, {
            headers: {
                'Authorization': `token ${GITHUB_CONFIG.token}`,
                'Content-Type': 'application/json',
                'User-Agent': 'Homestay-Map'
        ...
        if (!getResponse.ok) {
            throw new Error('Failed to get issue before update');
        }
